...
% "CHAPTER" size 16 (All Caps), Chapter Title size 14, heading 14, text 12, subheadings size 12(bold)
...
\vspace*{0.5cm} % Optional top spacing to push the title down slightly
...
\vspace{1cm} % Adds neat spacing between the text and the logo

% CUSAT Logo placed directly without the floating figure environment
...
Traditional biometrics lack inherent liveness detection, making the Electrocardiogram (ECG)—with its unique P, QRS, and T wave morphology dictated by individual cardiac anatomy—a highly secure alternative. The "CardioKey" project proposes a comprehensive biometric system utilizing an AD8232 ECG sensor and a precision analog front-end to amplify Lead I heart signals while rejecting 60Hz mains hum, which is then digitized at 300 Hz using a 16-bit ADS1115 ADC and an Arduino UNO data pipeline. Through Digital Signal Processing (DSP) involving wavelet drift correction and digital filtering, 150-sample P-QRS-T fragments are segmented per cardiac cycle and compressed into 30-point biometric signatures using Principal Component Analysis (PCA) to be evaluated using Linear Discriminant Analysis (LDA) and Nearest Mean classifiers. Validated on the public ECGID database of 45 subjects using Python, the system achieves up to 96\% identification accuracy under resting conditions, and by addressing heart rate variability through normalization and QT scaling, proves the ECG is a viable, universal, and unforgeable biometric trait for practical access control systems.
...
% Rename the default headings to ALL CAPS
...
% Adds List of Figures to the Table of Contents
...
% Adds List of Tables to the Table of Contents
...
\begin{flushleft}
% The @{} removes the invisible left margin, p{2.5cm} forces the first column to be exactly 2.5cm wide
\begin{tabular}{@{}p{2.5cm} l}
\textbf{ADC} & Analog-to-Digital Converter \\[0.2cm]
\textbf{DSP} & Digital Signal Processing \\[0.2cm]
\textbf{ECG} & Electrocardiogram \\[0.2cm]
\textbf{GUI} & Graphical User Interface \\[0.2cm]
\textbf{LDA} & Linear Discriminant Analysis \\[0.2cm]
\textbf{MCU} & Microcontroller Unit \\[0.2cm]
\textbf{PCA} & Principal Component Analysis \\[0.2cm]
\textbf{RLD} & Right Leg Drive \\[0.2cm]
...
\chapter{ANALOG–  CIRCUIT DESIGN AND TESTING}
\vspace{15pt}
First stage of the project strictly focused on the acquisition of the raw, biological analog signal. Because the electrical action potentials generated by the sinoatrial node of the heart dissipate through tissue, they arrive at the wrists with amplitudes of merely 1 millivolt ( $1 mV$ ). Extracting this signal requires immense amplification in an environment saturated with electromagnetic noise. \\ 
...
\section{LTspice Circuit Simulation}
Before moving to physical breadboard implementation, the complete analog front-end—comprising the AD620 instrumentation amplifier, the Right Leg Drive (RLD), and the filtering stages—was modeled and validated using LTspice. \\

To accurately test the circuit's amplification and noise rejection capabilities, a simulated AC voltage source was configured to mimic the biological 1 mV heart signal with noise. Additionally, massive 60 Hz mains interference and common-mode noise were intentionally injected into the simulation schematic.

% Insert your LTspice circuit schematic here
\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{imgs/circuitsim.jpeg} % Update with your actual schematic filename
    \caption{LTspice schematic of the AD620 amplifier and filtering stages simulating the ECG acquisition.}
    \label{fig:ltspice_circuit}
\end{figure}

The LTSpice simulation results validated the theoretical design calculations. The AC analysis demonstrated a deep attenuation curve exactly at the 60 Hz mark, confirming the effectiveness of the notch filter. Furthermore, the transient analysis (time-domain) output successfully showed the massive amplification of the simulated 1 mV heart signal into the target 1 V range while successfully clipping the high-frequency noise through the low-pass filter.

% Insert your LTspice output waveform plot here
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/out.jpg} % Update with your actual waveform filename
    \caption{LTspice transient and AC analysis output demonstrating 60Hz noise attenuation and signal amplification.}
    \label{fig:ltspice_output}
\end{figure}
...
As illustrated in the detailed system structure above, the biometric identification process is highly sequential, relying on strict feedforward links where the processed data output from one stage serves as the direct input to the subsequent stage. While the generic structure on the left outlines the core conceptual pipeline, the detailed architecture on the right specifies the exact complementary and alternative methods selected for the CardioKey system. The overall success of this biometric system depends on the careful synthesis, composition, and parameter adjustment of these mathematical methods, ensuring that the raw physiological data is perfectly refined before the final identification is executed.
...
The backend pairs an Arduino (C++) acquisition pipeline with a custom Python GUI, executing the biometric authentication via \textbf{Tkinter} and \textbf{OS} (interface), \textbf{PySerial} (hardware data), \textbf{NumPy} and \textbf{SciPy} (signal processing), \textbf{Scikit-Learn} (classification), and \textbf{Matplotlib} (visualization).
...
\section{Graphical User Interface (GUI) and System Integration}
To ensure the "CardioKey" system is user-friendly and operates as a seamless biometric lock, two dedicated Graphical User Interfaces (GUIs) were developed using Python's \textit{Tkinter} library. These interfaces eliminate the need for command-line interactions and provide real-time Matplotlib visualization natively embedded within the application windows.

\subsection{Dynamic Hardware ECG Acquisition (ecg\_record.py)}
The acquisition interface is designed to bridge the external Arduino hardware with the computer. Instead of hardcoding the sampling rate and duration into the Arduino firmware, this GUI allows dynamic configuration. 
\begin{itemize}
    \item \textbf{Dynamic Firmware Updating:} The user selects the COM port, sets the desired sampling rate (e.g., 300 Hz), and sets the recording duration. Upon clicking start, the Python script constructs a configuration string (e.g., "S,300,300") and transmits it over Serial. The Arduino dynamically recalculates its internal microsecond timer to execute the precise polling rhythm.
    \item \textbf{Multithreaded Acquisition \& Visualization:} The serial reading is isolated to a background thread to prevent the UI from freezing. Once the acquisition completes, the software instantly plots the first 5 seconds of the raw ECG signal to verify electrode quality.
\end{itemize}

\subsection{Biometric Identification Interface (ecg\_predict.py)}
The identification interface strictly handles the machine learning pipeline and user authentication. It is divided into two distinct operational phases:
\begin{itemize}
    \item \textbf{Model Training:} The system compiles the 150-sample P-QRS-T fragments from the selected database, applies PCA dimensionality reduction, and trains the Linear Discriminant Analysis (LDA) classifier.
    \item \textbf{Live Subject Prediction:} The user can upload a newly recorded live ECG file. The system processes the signal, extracts the features, and outputs the predicted Subject ID alongside a mathematical match confidence percentage. If no file is uploaded, the system utilizes a "Zero-Knowledge Auto-Demo" fallback, randomly extracting unseen tail-end data from the database to simulate a live hardware attempt and cross-checks the prediction accuracy.
\end{itemize}

% Insert your screenshot here
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/ecg_record.png} % Add your screenshot to imgs folder
    \caption{Dynamic Hardware ECG Acquisition Interface.}
    \label{fig:gui_record}
\end{figure}

% Insert your screenshot here
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/ecg_predict.png} % Add your screenshot to imgs folder
    \caption{ECG Biometric Identification Interface displaying prediction confidence and visualization.}
    \label{fig:gui_predict}
\end{figure}

\section{MCU Sampling Firmware (sampling\_firmware.ino)}
The Arduino UNO serves as the central microcontroller managing the high-speed data pipeline between the analog front-end and the external computer [1, 2]. To accurately capture the high-frequency R-peak and preserve the structural integrity of the entire cardiac cycle, the project methodology requires a strict digital sampling rate of exactly 300 Hz [3, 4]. 

To achieve this without the time drift associated with standard blocking functions, the firmware utilizes a precision hardware-level microsecond timer. Key features of this embedded implementation include:

\begin{itemize}
    \item \textbf{ADC Data Rate Configuration:} The external 16-bit ADS1115 Analog-to-Digital Converter is explicitly configured to operate in continuous mode with its Data Rate register set to 860 Samples Per Second (SPS). Operating the hardware at 860 SPS guarantees that the ADC digitizes the analog voltage significantly faster than the microcontroller's 300 Hz polling cycle, preventing the retrieval of duplicate or stale voltage readings and providing a highly reliable data pipeline.
    \item \textbf{Dynamic Command Parsing:} Rather than hardcoding the 300 Hz sampling rate and 5-minute duration limit into the C++ sketch, the firmware actively listens for a comma-separated configuration string (e.g., "S,300,300") sent over the Serial port by the Python GUI. 
    \item \textbf{Microsecond Precision Polling:} Upon receiving the configuration string, the microcontroller dynamically calculates the exact microsecond delay required between readings (e.g., 3,333 microseconds for 300 Hz). It executes a non-blocking timer to strictly poll the I2C bus and stream the digital voltages to the computer without drifting over the 90,000-sample recording session.
\end{itemize}
...
\item \textbf{Dry Electrode Transition:} Fully phase out the medical Ag/AgCl sticky patches used in prototyping in favor of stainless steel or copper touch-pads to allow seamless, instant user interaction.
...
\section{Project Repository and Documentation}
To ensure full transparency and reproducibility of the "CardioKey" system, all supplementary project files have been consolidated and maintained in an online digital version control repository. 

This repository contains the complete source code for the Arduino embedded firmware, dsp \& classification and the Python Graphical User Interfaces (GUIs), as well as the circuit simulations utilized during the design of the analog front-end. Furthermore, it hosts the extended experimental result datasets (including raw ECG data logs from the hardware testing phases) and comprehensive system documentation.

The complete project repository can be accessed via the following link:

\vspace{10pt}
\begin{center}
    \textbf{Project Repository:} \texttt{https://github.com/aaromalonline/cardiokey} % Replace with your actual repo URL
\end{center}
\vspace{10pt}

The subsequent sections within this appendix highlight the core programmatic excerpts of the firmware, signal processing, and machine learning classification pipelines referenced throughout this report, through sample code.
...
\section{Dataset Iteration and Pipeline Execution}
The final section of the pipeline iterates through the dataset files (1.txt to 90.txt), mathematically determines the subject ID and activity state based on the odd/even file naming convention, processes the raw signals, and executes the authentication model.
...
if __name__ == "__main__":
    ...
    for i in range(1, 91):
        ...
        # Odd files = Rest, Even files = Exercise
        is_exercise = (i % 2 == 0) 
        ...
        beats = segment_beats(clean_signal, fs=300, 
                              num_beats=10)
    ...
    if len(dataset_X_rest) > 0:
        ...
        # Optional: Generate visualization for Subject 1
        # plot_signal_conditioning(raw_signal, clean_signal, 
        #                          beats, subject_id=1)
\end{verbatim}

\section{Arduino ECG Sampling Firmware}
This C++ firmware runs on the Arduino UNO. It configures the ADS1115 to 860 SPS and awaits a dynamic configuration string from the Python GUI to set the microsecond timer for precise sampling.
\begin{verbatim}
#include <Wire.h>
...
\newpage
% Renames the heading to ALL CAPS and forces it to exactly 16pt
\renewcommand{\bibname}{\fontsize{16}{19}\selectfont BIBLIOGRAPHY} 
